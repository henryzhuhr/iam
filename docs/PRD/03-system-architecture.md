...
- **IAM 服务层**：IAM 核心服务模块
  - 认证服务：用户登录、邀请注册/激活、内部客户端认证、Token 生成与验证
...
| **客户端 (Client)** | 平台统一注册的内部系统身份，用于机器对机器认证 | client_id、allowed_scopes、状态 |
...
| **用户 (User)** | 属于某个租户的具体个人，通过邀请注册/激活加入租户，可被授权访问一个或多个应用 | user_id、邮箱、角色列表 |
...
1. 租户创建在前，用户注册在后；注册流程本身不负责创建租户
2. 首版本用户注册必须携带租户上下文，通过邀请链接或预创建账号激活完成
3. 用户只能归属一个租户，因此同一条注册链接只能绑定一个 `tenant_id`
4. 邮箱唯一性按租户维度生效，即 `(tenant_id, email)` 唯一，跨租户允许重复
5. 匿名开放注册不纳入当前版本，后续如需支持应单独定义“租户自助开通”流程
...
| **纯 RBAC v1** | v1 不支持用户直接权限、用户组权限、拒绝策略 |
| **应用范围附加** | 角色定义仍为租户级，用户分配角色时可附加应用范围 |
...
| 本文档章节 | 关联文档 |
|------------|----------|
| 3.2 核心数据模型 | [REQ-017 应用管理与数据隔离功能](./05-functional-requirements/REQ-017-application-isolation.md) |

...
| **应用 (Application)** | 租户下的业务系统，如 OA、CRM、ERP 等 | app_code、名称、状态 |
...
所有 API 查询必须通过中间件自动注入 `tenant_id` 过滤条件：
...
-- 正确：强制带 tenant_id
SELECT * FROM users WHERE id = 1001 AND tenant_id = 100;
```

### 3.3.3 应用隔离实现

业务系统数据表需要增加 `app_id` 字段进行应用隔离：
...
CREATE TABLE business_records (
    ...
    app_id BIGINT NOT NULL,      -- 应用隔离
    user_id BIGINT,
    data JSON,
    created_at DATETIME
);

-- 查询时必须同时带 tenant_id 和 app_id
SELECT * FROM business_records
WHERE tenant_id = 100
  AND app_id = 1001
  AND user_id IN (SELECT user_id FROM user_apps WHERE app_id = 1001);
```
...
```
用户请求 → API 网关 → 认证中间件 → 权限中间件 → 业务逻辑 → 数据访问
              ↓            ↓            ↓
          限流降级     Token 验证    RBAC 校验
```

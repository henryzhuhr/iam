...
在 IAM 中增加 `Application` 实体，作为租户下的资源进行统一管理。
...
CREATE TABLE iam_application (
    ...
    name        VARCHAR(64) NOT NULL,      -- 应用名称（如：OA、CRM）
    app_code    VARCHAR(32) NOT NULL,      -- 应用编码（唯一标识）
...
| REQ-012 Token 管理 | 可选：Token 增加 `apps` claim |
